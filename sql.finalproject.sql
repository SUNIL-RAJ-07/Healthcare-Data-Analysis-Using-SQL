...
#bill table
SELECT SUM(bill_amount) AS total_revenue FROM billing_detials;

SELECT AVG(bill_amount) AS average_bill FROM billing_detials;

SELECT payment_mode, SUM(bill_amount) AS total_revenue FROM billing_detials GROUP BY payment_mode;

SELECT payment_mode, COUNT(*) AS total_transactions FROM billing_detials GROUP BY payment_mode;

SELECT MAX(bill_amount) AS highest_bill,
       MIN(bill_amount) AS lowest_bill FROM billing_detials;
       
SELECT MONTH(payment_date) AS month, SUM(bill_amount) AS total_revenue FROM billing_detials 
GROUP BY MONTH(payment_date) ORDER BY month;

SELECT * FROM billing_detials WHERE bill_amount > (SELECT AVG(bill_amount) FROM billing_detials);

SELECT * FROM billing_detials ORDER BY bill_amount DESC
LIMIT 10;
 #combined(joins)
 SELECT p.city,SUM(b.bill_amount) AS total_revenue FROM patient_detials p 
 INNER JOIN appointments a ON p.patient_id = a.patient_id
 INNER JOIN billing_detials b ON a.appointment_id = b.appointment_id 
